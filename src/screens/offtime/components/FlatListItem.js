...
import { deleteDayOffApi }   	from '../../../app/api/offtime';
...
class FlatListItem extends Component {
    ...
    showDetail = () => {
        ...
        if (this.props.item.off_type == OFFTIME_TYPE.Flextime) {
            ...
            route = 'EditDayOff';
            params = {
                dayoff_id: this.props.item.target_id,
            };
        }
	...
	deleteDetail = async() => {
		if (this.props.item.off_type == OFFTIME_TYPE.Flextime) {
			...
			let data = {
				id      :   this.props.item.target_id,
				user 	: 	this.props.userInfo.user_id,
				branch  :   this.props.branch.branch_id,
			}
			let response    =   await deleteDayOffApi(data);
			console.log('-----------------------');
			   ...
			   right: [
				   ...
				   },
				   {
                    //    onPress: this.deleteDetail,
                        onPress: () => Alert.alert(
                                'Confirm',
                                'Are you sure to delete this '+(this.props.item.off_type == OFFTIME_TYPE.Flextime?'flextime?':'day off?'),
                                [
