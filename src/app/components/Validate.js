...
const funcConvertDate = (date) => {
    return parseInt(_.isDate(date) ? moment(date).format('YYYYMMDDHHmm') : '');
}

//ANS848 - TriVH - 2019/01/12 check compare with other value
export const validateCompareOtherDate = (otherDateList) => {
    let errorMessage = undefined;
    if (otherDateList.length > 0) {
        otherDateList.forEach(function(dateFst) {
            let tmpDateFst = funcConvertDate(dateFst[0]);
            let count = 0;
            otherDateList.forEach(function(dateSnd) {
                let tmpDateSnd = funcConvertDate(dateSnd[0]);
                if(tmpDateFst == tmpDateSnd) {
                    ++count;
                    if(count == 2) {
                        errorMessage = 'It has been selected!';
                    }
                }
            });
        });
